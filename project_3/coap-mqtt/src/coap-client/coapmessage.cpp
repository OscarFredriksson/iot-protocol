...
std::ostream& operator<<(std::ostream& os, const CoapMessage::Option& rhs) {
  ...
  case ContentFormat:
    if (rhs.value.empty())
      os << CoapContentFormat::text;
    else {
      uint16_t val = rhs.value[0];

      if (rhs.value.size() > 1)
        val = val << 8 | rhs.value[1];
  ...
  case LocationPath:
  case UriPath:
    for (auto c : rhs.value)
      os << c;
    break;
  default:
    for (auto c : rhs.value)
      os << std::hex << (0xff & static_cast<int>(c)) << " ";
    break;
  }
...
int CoapMessage::deserialize(const std::vector<char>& msg) {
  ...
  byteIt +=
      2 +
      tokenLength; // Increment 2 for message Id, skip tokens and go to option
  ...
  for (; byteIt != msg.size() - 1; byteIt++) {
    ...
    if ((msg[byteIt] & 0xff) == 0xff) {
      ...
      Option option;
      option.delta =
          static_cast<CoapOptionDelta>(prevDelta + ((msg[byteIt] & 0xf0) >> 4));
...
std::vector<char> CoapMessage::serialize() {
  std::vector<char> msg = {
      // Bitshift version and type to place them at the correct places in the
      // byte, use the or-operator to append them together with the token length
      char((version << 6) | (type << 4) | tokenLength),
      char(code),
      // The message Id requires two bytes, add the 8 most significant bits to
      // one byte and the 8 least significant to another
      char(messageId >> 8),
      char(messageId & 0xff),
  };
  ...
  for (int i = 0; i < options.size(); i++) {
    ...
    // Go through every character in the value string and add them to the byte
    // array
